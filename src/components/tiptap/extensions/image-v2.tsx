...
export const ImageExtension = Image.extend({
    addAttributes() {
        return {
            ...
            inline: {
                default: this.options.inline,
                parseHTML: (element) => element.hasAttribute('inline'),
...
function TiptapImage(props: NodeViewProps) {
    ...
    const [isInline, setIsInline] = useState(node.attrs.inline || false);
    ...
    function resize(event: MouseEvent) {
        ...
        if (isInline || newWidth < parentWidth) {
            updateAttributes({
                width: newWidth,
            });
        }
    ...
    function toggleInline() {
        const newInlineValue = !isInline;
        setIsInline(newInlineValue);
        updateAttributes({ inline: newInlineValue });

        // When toggling inline, also update any relevant styles
        if (newInlineValue) {
            // For inline images, we might want to reset some attributes
            updateAttributes({
                align: 'center',
                width: 'auto',
            });
    ...
    return (
        <NodeViewWrapper
            ...
            className={cn(
                ...
                !node.attrs.inline && node.attrs.align === 'left' && 'left-0 -translate-x-0',
                !node.attrs.inline && node.attrs.align === 'center' && 'left-1/2 -translate-x-1/2',
                !node.attrs.inline && node.attrs.align === 'right' && 'left-full -translate-x-full',
                node.attrs.inline && 'inline-block align-middle mx-1'
            )}
        ...
        >
            <div className={cn('group relative flex flex-col rounded-md', resizing && '')}>
                <figure className="relative m-0">
                    <img
                        ...
                        className={cn(
                            ...
                            node.attrs.inline && 'inline-block'
                        )}
                        style={{
                            verticalAlign: node.attrs.inline ? 'middle' : undefined,
                        }}
                        onLoad={(e) => {
                ...
                {editor?.isEditable && (
                    <div
                        className={cn(
                            'absolute right-4 z-40 top-4 flex items-center gap-1 rounded-md border bg-background/80 p-1 opacity-0 backdrop-blur transition-opacity',
                    ...
                    >
                        ...
                        <DropdownMenu open={openedMore} onOpenChange={setOpenedMore}>
                            ...
                            <DropdownMenuContent align="start" alignOffset={-90} className="mt-1 text-sm">
                                ...
                                <DropdownMenuItem onClick={toggleInline}>
                                    <span className="mr-2 size-4 flex items-center justify-center">⟷</span>
                                    {node.attrs.inline ? 'Make Block' : 'Make Inline'}
                                </DropdownMenuItem>
                                {!node.attrs.inline && (
                                    <DropdownMenuItem
                                        onClick={() => {
